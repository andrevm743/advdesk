...
exports.generatePetition = generatePetition;
exports.generateJudgeReport = generateJudgeReport;
...
async function generatePetition(input) {
...
Redija a petição completa, detalhada e pronta para protocolo.`;
    v2_1.logger.info("Generating petition with Claude Sonnet");
...
async function generateJudgeReport(input) {
    const answersText = Object.entries(input.strategicAnswers)
...
${input.petitionContent}`;
    v2_1.logger.info("Generating judge report with Claude Sonnet");
    const message = await getClient().messages.create({
        model: SONNET_MODEL,
        max_tokens: 6144,
        system: systemPrompt,
...
async function generateChatResponse(systemContext, history, userMessage, fileContext, customSystemPrompt) {
    const messages = [
