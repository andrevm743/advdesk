...
exports.analyzeInitialCase = analyzeInitialCase;
exports.buildPetitionStructure = buildPetitionStructure;
exports.analyzeForJudge = analyzeForJudge;
exports.generateChatReport = generateChatReport;
...
async function fileToGeminiPart(storagePath, mimeType) {
...
async function transcribeAudio(storagePath, mimeType) {
...
async function analyzeInitialCase(input) {
        ...
        for (const path of input.knowledgeBasePaths) {
            ...
            if (!isAudio(mimeType)) {
                try {
                    ...
                    v2_1.logger.warn(`Failed to process KB file ${path}:`, err);
                }
            }
        }
    }
    ...
    for (const path of input.storagePaths) {
        ...
        if (isAudio(mimeType)) {
            ...
            try {
                ...
                v2_1.logger.warn(`Failed to process file ${path}:`, err);
            }
        }
    }
...
async function buildPetitionStructure(input) {
    const model = getGenAI().getGenerativeModel({
        model: FLASH_MODEL,
        generationConfig: { responseMimeType: "application/json" },
    });
...
async function analyzeForJudge(input) {
    const model = getGenAI().getGenerativeModel({
        model: FLASH_MODEL,
        generationConfig: { responseMimeType: "application/json" },
    });
    ...
    if (input.petitionFileStoragePath) {
        ...
        try {
            ...
            v2_1.logger.warn(`Failed to process petition file ${input.petitionFileStoragePath}:`, err);
        }
    }
    for (const path of input.storagePaths) {
        const mimeType = getMimeType(path);
...
async function generateChatReport(clientName, area, messages) {
    const model = getGenAI().getGenerativeModel({
        model: FLASH_MODEL,
        generationConfig: { responseMimeType: "application/json" },
    });
